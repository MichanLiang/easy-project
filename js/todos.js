...
// 即時監聽成員的 todos subcollection
let todoUnsubscibes = [];
function setupTodoListeners(){
  todoUnsubscibes.forEach(u => u());
  todoUnsubscibes = [];
  ...
  for(const member of acceptedMembers){
    // 監聽成員的 todos subcollection：指派給我的任務
    const unsub = firebase.firestore().collection('users').doc(member.id).collection('todos')
      ...
      .onSnapshot(snap => {
        ...
        const after = JSON.stringify(DB.todos);
        if(before !== after){ persist(); render(); }
      });
    todoUnsubscibes.push(unsub);

    // 監聽我指派給這個成員的任務：讀取對方 subcollection 的最新狀態
    const myTasks = DB.todos.filter(t => t.assignedBy === user.uid && t.assignee === member.id);
      ...
      const unsub2 = firebase.firestore().collection('users').doc(member.id).collection('todos')
        ...
        .onSnapshot(doc => {
          ...
          if(local && (local.status !== remoteData.status || local.title !== remoteData.title)){
            local.status = remoteData.status;
            local.title = remoteData.title || local.title;
            local.date = remoteData.date || local.date;
            local.note = remoteData.note || local.note;
            persist(); render();
          }
        });
      todoUnsubscibes.push(unsub2);
    }
  }
}
